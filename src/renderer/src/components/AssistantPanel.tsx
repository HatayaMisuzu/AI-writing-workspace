...
import type { AIMode, ChatMessage, ContextBundle, DigestStatus, MemoryProposal, Project } from '../../../shared/domain'
import type { EditorSelection } from './EditorSurface'

type ContextSummary = { count: number; tokens: number; items: Array<{ kind: string; title: string; reason: string }> }
type ViewMessage = {
  id: string; role: 'user' | 'assistant'; content: string; status?: 'streaming' | 'error' | 'cancelled'; mode?: AIMode
  createdAt: number; selection?: EditorSelection; proposals?: MemoryProposal[]; proposalError?: string; recorded?: boolean
  context?: ContextSummary; inserted?: boolean
}
...
const historyMessage = (message: ChatMessage): ViewMessage => ({ id: message.id, role: message.role,
  content: message.content || (message.status === 'cancelled' ? '上次请求已取消。' : message.status === 'error' ? '上次请求未完成。' : ''),
  status: message.status === 'streaming' || message.status === 'error' || message.status === 'cancelled' ? message.status : undefined,
  mode: message.taskMode, createdAt: message.createdAt })
...
export function AssistantPanel({ project, documentId, documentRevision, selection, collapsed, onCollapse, onInsertCandidate, onCreatePatch, onBeforeAI, onOpenSettings }: {
  ...
  const loadThread = useCallback(async (): Promise<void> => {
    ...
    const threads = await window.workspace.chat.listThreads(project.id)
    const thread = threads[0] ?? await window.workspace.chat.newThread(project.id, '创作讨论')
    const history = await window.workspace.chat.listMessages(project.id, thread.id, undefined, 50)
    setThreadId(thread.id); setMessages(history.map(historyMessage)); setHasEarlier(history.length === 50); setLoadingHistory(false)
  }, [project.id])
  ...
  useEffect(() => { void window.workspace.providers.models().then((models) => setModelName(models.find((item) => item.isDefault && item.enabled)?.displayName ?? '未配置模型')) }, [])
  useEffect(() => {
  ...
  const loadEarlier = async (): Promise<void> => {
    ...
    setMessages((current) => [...earlier.map(historyMessage), ...current]); setHasEarlier(earlier.length === 50)
  }
  const newThread = async (): Promise<void> => {
    if (runningId) return
    const thread = await window.workspace.chat.newThread(project.id, '新对话')
    setThreadId(thread.id); setMessages([]); setHasEarlier(false); setInput('')
  }
  ...
  const submit = async (): Promise<void> => {
    ...
    setMessages((current) => [...current, { id: messageId, role: 'user', content: text, mode: requestedMode, selection, createdAt: now },
      { id: assistantId, role: 'assistant', content: '', status: 'streaming', mode: requestedMode, selection, createdAt: now + 1 }])
    setInput('')
    ...
    const cleanup = window.workspace.ai.start({ requestId, threadId, userMessageId: messageId, assistantMessageId: assistantId, task: {
      ...
      if (event.type === 'chunk') {
        ...
        const context = bundle ? { count: bundle.items.length, tokens: bundle.metadata.estimatedTokens,
          items: bundle.items.map((item) => ({ kind: item.kind, title: item.title, reason: item.reason })) } : undefined
  ...
  return <aside className="assistant-panel">
    ...
    <div className="chat-scroll">
      ...
      {loadingHistory ? <div className="chat-loading">正在读取这部作品的对话…</div> : messages.length === 0 ? <div className="chat-empty"><Sparkles size={23} /><h3>从这部作品继续聊</h3><p>讨论不会自动改正文；确定的设定会先成为待确认提案。</p>{modelName === '未配置模型' && <button className="text-action" onClick={onOpenSettings}>配置模型与服务</button>}</div> : messages.map((message) => <div key={message.id} className={`message ${message.role} ${message.status ?? ''}`}>
        ...
        {message.context && <details className="context-inspector"><summary>使用 {message.context.count} 条作品上下文 · 约 {message.context.tokens} tokens</summary><ul>{message.context.items.map((item, index) => <li key={`${item.kind}-${index}`}><strong>{item.title}</strong><span>{item.reason}</span></li>)}</ul></details>}
        ...
        {message.role === 'assistant' && message.content && <div className="message-actions"><button title="复制" onClick={() => void navigator.clipboard.writeText(message.content)}><Copy size={14} /></button>
          {message.mode === 'generation' && !message.status && !message.inserted && <button onClick={() => void onInsertCandidate(message.content).then(() => setMessages((current) => current.map((item) => item.id === message.id ? { ...item, inserted: true } : item))).catch((error) => setMessages((current) => current.map((item) => item.id === message.id ? { ...item, content: `${item.content}\n\n插入失败：${error instanceof Error ? error.message : String(error)}`, status: 'error' } : item)))}><PenLine size={14} />插入候选</button>}
