...
export function ProjectSidebar({ project, tree, selectedId, section, onSection, onSelect, onNewVolume, onNewChapter, onReorder }: {
  ...
  return <aside className="project-sidebar">
    ...
    {section === 'manuscript' ? <div className="manuscript-tree">
      {volumes.map((volume) => <div className="volume" key={volume.id}>
        <div className="volume-row"><ChevronDown size={16} /><strong>{volume.title}</strong><span /><button className="icon-button" onClick={() => onNewChapter(volume.id)} title="新建章节"><Plus size={15} /></button></div>
        {tree.filter((node) => node.parentId === volume.id).map((node, index) => <button key={node.id} draggable className={node.id === selectedId ? 'chapter-row active' : 'chapter-row'} onClick={() => onSelect(node)}
          ...
          <FileText size={14} /><span className="chapter-title">{node.title}</span><small>{node.wordCount.toLocaleString()}</small>
        </button>)}
      </div>)}
