...
- 可持久化 AI 对话、显式章节理解状态、保守的记忆意图识别、来源安全的风格样本
- 本地规则校对、可审阅 AI 校对提案、历史版本差异与可恢复修改
...
```powershell
npm run verify  # 类型检查、完整本地测试、生产构建
...
- AI 上下文构建、对话、记忆、摘要与风格样本都必须携带 `projectId`，不同作品不会互相检索或注入。
